	...
	<div class="container">
		<div class="content">
			...
			<form class="form-inline" method="get">
				<div class="form-group">
					<select name="urut" class="form-control" onchange="form.submit()">
						...
						<option value="2" <?php if($urut == '2'){ echo 'selected'; } ?>>Sudah Transfer</option>
					</select>
				</div>
			</form>
			<br />
			...
			<table class="table table-striped table-hover">
				...
				<?php
				if($urut){
					$sql = mysqli_query($koneksi, "SELECT * FROM konfirmasi WHERE status='$urut' ORDER BY id_ref ASC");
				}else{
					$sql = mysqli_query($koneksi, "SELECT * FROM konfirmasi ORDER BY id_ref ASC");
				}
				...
				<td>';if ($row['status']==1) {
					echo '<a href="up.php?id='.$row['id_ref'].'" title="Rubah Data"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span>KONFIRMASI   </a>';
					}else{
				...
				<td>';
							if ($row['status']==2) {
								echo '<a href="down.php?id='.$row['id_ref'].'" title="Rubah Data"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span>   CANCEL</a>';
							}else{
								echo "   Belum";
							}'
							</td>
